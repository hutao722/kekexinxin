...
import JMeter.plugins.functional.samplers.websocket.custom.MessageHandler;
...
public class ServiceSocket {
    ...
    public void onMessage(byte buf[], int offset, int length) {
    	synchronized (parent) {
            logMessage.append(" - Received message #").append(messageCounter).append(length);
            String msg = MessageHandler.getJsonFromResult(buf);
    ...
    public void onOpen(Session session) {
        ...
        int idleTimeout = 150;
        if (parent.getProxyPort() != null) {
        	try {
        		idleTimeout = Integer.parseInt(parent.getProxyPort());
        	} catch (Exception e) {
        		idleTimeout = 150;
        	}
        	this.session.setIdleTimeout(idleTimeout * 1000);
        } else {
        	this.session.setIdleTimeout(idleTimeout * 1000);
        }
        connected = true;
    ...
    public boolean awaitOpen(String connectionId, int duration, TimeUnit unit) throws InterruptedException {
    ...
    /**
     * tanster json to pb data
     * @param message
     * @throws IOException
     */
    public void sendMessage(String message) throws IOException {
    	if (session == null || !session.isOpen()) {
    		log.error("session is not ready!");
    		return;
    	}
        session.getRemote().sendBytesByFuture(MessageHandler.getPBData(message));
    }
